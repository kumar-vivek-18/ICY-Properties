...
const PropertyDetailItems = ({ property }) => {
  ...
  const verifyProperty = async () => {
    try {
      await axios.post('http://80.65.208.60:5000//v1/property/update-property-status', {
        ...
        .then((res) => {
          if (res.status === 200) {
            ...
            const remainingProperties = properties.filter(curr => curr._id != property._id);
      ...
      console.error("Error occured while updating property status", error.message);
    }
  }
    ...
    <Fragment>
      <div className="w-full lg:w-auto m-0  flex flex-col bg-white dark:bg-black rounded-3xl  shadow-md my-[100px]">
        <div>
          ...
          {property?.verified === "unverified" && <div className="flex flex-row justify-center py-10">
            <button onClick={() => { verifyProperty() }} className="py-4 px-60 bg-yellow rounded-xl text-white font-bold text-[18px]">Verify Property</button>
          </div>}
        </div>
        {verifyModal && <div className="w-1/3 h-1/2 bg-white shadow-2xl fixed top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 flex flex-col justify-center items-center rounded-2xl">
          <div className="flex flex-col justify-center items-center">
            <img src="https://res.cloudinary.com/kumarvivek/image/upload/v1730718322/success_dxtj1y.png" alt="success" width={140} height={140} />
            <div className="text-3xl text-green font-bold mt-6">Property Verified</div>
